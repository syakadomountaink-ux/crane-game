...
# --- 物理逆算ソルバー ---
def calc_dynamics(T, t_d, hook_clock, D_hook, v_y_cm_s, L_cm, A_x_cm):
...
u_x_a, u_y_a, aim_x, aim_y, req_x_a, swing_y_a, push_x_a, dir_a, possible_a = calc_dynamics(T_auto, t_d, hook_clock, D_hook, v_y_cm_s, L_cm, A_x_cm)
...
u_x_m, u_y_m, _, _, req_x_m, swing_y_m, push_x_m, dir_m, possible_m = calc_dynamics(T_manual, t_d, hook_clock, D_hook, v_y_cm_s, L_manual_cm, A_x_cm)
...
max_r = max(10, abs(u_x_a)+5, abs(u_y_a)+5, D_hook+5, abs(swing_y_a)+5, A_x_cm+5)
...
ax2d.plot(0, 0, marker='*', color='gold', markersize=30, markeredgecolor='black', label="フック (0,0)")
ax2d.arrow(0, 0, aim_x*0.5, aim_y*0.5, head_width=1.0, head_length=1.5, fc='gold', ec='orange', linewidth=2, alpha=0.5)
...
if T_auto > 0 and possible_a:
    ...
    ax2d.plot(aim_x, aim_y, marker='o', color='lightcoral', markersize=10, label="目標着地点")
