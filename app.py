...
if mode == "自動計算 (チェーン＋リング)":
    ...
    # リングの質量計算 (こちらは鉄の密度 7.8g/cm³ とトーラス体積で計算)
    density_steel = 7.8
    d_ring_fixed = 4.0 / 10.0 # 線の太さ 4mm 固定
    ...
    calc_info = f"実測値ベース (チェーン約 {m_chain:.1f}g / リング約 {m_ring:.1f}g)"
...
if L_m > 0:
    ...
    * 揺れの周期: 約 **{T:.2f} 秒**
    ...
    y_pos_text = "奥" if y_pos > 0 else "手前" if abs(y_pos) > 0.01 else "中心付近"
